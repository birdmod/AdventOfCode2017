...
let input = "3751	3769	2769	2039	2794	240	3579	1228	4291	220	324	3960	211	1346	237	1586
...
let findRowDelta (xs:int []) = 
    (Array.max xs) - (Array.min xs)

let checksum = extractedNums |> Array.sumBy findRowDelta
    
(* part 2*)

let findRowDeltaWithDivision (xs:int []) = 
    let findCouple (xs:int[]) candidate =
        (* should be ok because we suppose the number do not appear twice on a row *)
        (* BTW if it divides it implies that x > candidate *)
        let other = xs |> Array.tryFind (fun x -> (x <> candidate) && (x % candidate = 0))
        match other with
        | None -> None
        | Some(v) -> Some(v, candidate)

    [| for cnd in xs -> findCouple xs cnd |]

let evenlyDivideCouples = 
    extractedNums 
    |> Array.map findRowDeltaWithDivision
    |> Array.collect (fun line -> line |> Array.choose id)
    |> Array.map (fun couple -> fst couple / snd couple)
    |> Array.sum